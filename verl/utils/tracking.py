...
class _TensorboardAdapter:
    ...
    def log(self, data, step):
        ...
        for key in data:
            value = data[key]
            # Handle histogram data (keys starting with 'hist/')
            if key.startswith('hist/') and isinstance(value, list) and len(value) > 0:
                # Remove 'hist/' prefix for cleaner tensorboard naming
                hist_key = key[5:]  # Remove 'hist/' prefix
                try:
                    self.writer.add_histogram(hist_key, np.array(value), step)
                except Exception:
                    # Fallback to scalar mean if histogram fails
                    if isinstance(value[0], (int, float)):
                        self.writer.add_scalar(hist_key + '/mean', float(np.mean(value)), step)
            # Handle scalar values
            elif isinstance(value, (int, float)):
                self.writer.add_scalar(key, value, step)
            # Handle list values (convert to mean for scalar logging)
            elif isinstance(value, list) and len(value) > 0 and isinstance(value[0], (int, float)):
                self.writer.add_scalar(key, float(np.mean(value)), step)
