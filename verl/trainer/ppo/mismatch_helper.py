...
def compute_rollout_importance_weights(
    ...
    Returns:
        Tuple of (weights_proto, metrics) where:
            ...
            metrics: Dictionary of IS statistics and mismatch metrics (KL, PPL, etc.),
                all converted to scalars and prefixed with "mismatch/". Examples:
                - IS metrics: "mismatch/rollout_is_mean", "mismatch/rollout_is_max"
                - Mismatch metrics: "mismatch/kl", "mismatch/training_ppl", "mismatch/rollout_ppl"
    """
    ...
    # Handle string "null" from Hydra config (convert to None)
    if rollout_is_threshold_lower is not None and isinstance(rollout_is_threshold_lower, str):
        if rollout_is_threshold_lower.lower() in ("null", "none", ""):
