...
class RewardManager():
    ...
    def __call__(self, data: DataProto):
        ...
        for i in range(len(data)):
            ...
            if data_source in ['webqsp', 'grailqa', 'graphq']:
                # For S-Expression data, use mid_reward with format reward and progressive scaling
                reward_dict = mid_reward.compute_mid_reward(
                    solution_str=sequences_str, 
                    ground_truth=list(ground_truth['target']), 
                    structure_format_score=self.structure_format_score, 
                    correct_reward=self.mid_f1_weight,
                    training_step=self.training_step
                )
                base_score = reward_dict.get('total', 0.0)
                # record components for logging
                mid_f1_list.append(float(reward_dict.get('mid_f1', 0.0)))
                structure_reward_list.append(float(reward_dict.get('structure_reward', 0.0)))
            else:
                raise
...
class TaskRunner:
    ...
    def run(self, config):
        ...
        # )
        reward_fn = RewardManager(tokenizer=tokenizer, num_examine=0, structure_format_score=config.reward_model.structure_format_score, mid_f1_weight=getattr(config.reward_model, 'mid_f1_weight', 1.0))
        val_reward_fn = RewardManager(tokenizer=tokenizer, 
                                       num_examine=3,  # 验证时输出较少样本
                                       structure_format_score=0.0,  # 验证时不加格式奖励
                                       mid_f1_weight=1.0)
