...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        logger.info("************Start generate_sequences in rollout*************")
        ...
        meta_info = {
            ...
            'stop_sequences': ['</sparql>', '</action>']  # Add </sparql> and </action> as stop sequences
        }
        prompts.meta_info.update(meta_info)

        timing_generate = {}
        logger.info("************Before switch to rollout mode*************")
        ...
        logger.info("************After switch to rollout mode*************")
        logger.info("************Before generate_sequences*************")
        with simple_timer("generate_sequences", timing_generate):
            output = self.rollout.generate_sequences(prompts=prompts)
        logger.info("************After generate_sequences*************")
        if self._is_actor:
            loop.run_until_complete(self.trainer_mode())
        ...
        logger.info("************End generate_sequences in rollout*************")
        return output
